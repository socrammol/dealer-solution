...
public class ProductRepositoryold {
  ...
  public Map<Integer, String[]> todos(){
  ...
  public Map<Integer, String[]> retrieveUnavailable(String id){
    final HashMap<Integer, String[]> unavailableProductList = new HashMap<>();
    for (Entry<Integer, String[]> product : map_produtos.entrySet()) {
      if (id.equals(product.getKey().toString())) {
  ...
  public void save(String[] novoProduto){
    map_produtos.put(map_produtos.size()+1, novoProduto);
  }
